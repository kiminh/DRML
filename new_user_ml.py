...
def data_generation(active_user_dict,active_label_dict,movie_dict):
    ...
    for user, item, labels in zip(active_user_dict.keys(), active_user_dict.values(), active_label_dict.values()):
        ...
        if n_user<=100:
            ...
            test_sup_tensor = dataset_prep(train_movie[15:], movie_dict)
            test_sup_set_x.append(test_sup_tensor)

            test_sup_label = torch.unsqueeze(torch.tensor(train_label[15:]).float(), 1)
            test_sup_set_y.append(test_sup_label)
            test_que_label = torch.unsqueeze(torch.tensor(train_label[:15]).float(), 1)
            test_que_set_y.append(test_que_label)

            test_que_tensor = dataset_prep(train_movie[:15], movie_dict)
